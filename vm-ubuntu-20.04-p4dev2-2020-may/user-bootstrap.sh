...
patch -p1 < ~/behavioral-model-skip-python-pip.patch
./install_deps.sh
cd ..
...
# Versions of this script for Ubuntu releases before Ubuntu 20.04
# installed the package python-ipaddr.  However, that no longer exists
# in Ubuntu 20.04.  PIP for Python2 can install the ipaddr module,
# which is good enough to enable two of p4c's many tests to pass,
# tests that failed if the ipaddr Python 2 module is not installed, in
# my testing on 2020-Apr-24.  From the Python stack trace that appears
# when running those failing tests, the code that requires this module
# is in behavioral-model's runtime_CLI.py source file, in a function
# named ipv6Addr_to_bytes.
sudo -H pip install ipaddr==2.2.0
